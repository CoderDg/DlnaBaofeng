...
public class ControlActivity extends BaseActivity implements OnClickListener {
    ...
    private List<String> urls = new ArrayList<String>(){{
        add("http://video.dispatch.tc.qq.com/11843129/v0015ia0zha.mp4");
    }};
    ...
    private synchronized void getMaxVolumn() {
        new Thread() {
            public void run() {
                final int maxVolumnValue = mController.getMaxVolumeValue(mDevice);

                if (maxVolumnValue <= 0) {
                    LogUtil.d(TAG, "get max volumn value failed..");
                    sb_voice.setMax(100);
                } else {
                    LogUtil.d(TAG, "get max volumn value success, the value is " + maxVolumnValue);
    ...
    private synchronized void goon(final String pausePosition) {
        new Thread() {
            @Override
            public void run() {
                final boolean isSuccess = mController.goon(mDevice, pausePosition);
                if (isSuccess) {
                    mPlaying = true;
    ...
    private synchronized void getPositionInfo() {
        new Thread() {
            ...
            public void run() {
                ...
                LogUtil.d(TAG, "嘟嘟嘟嘟嘟嘟嘟嘟嘟嘟嘟嘟嘟Get position info and the value is " + position);
                if (TextUtils.isEmpty(position) || NOT_IMPLEMENTED.equals(position)) {
                    return;
                }
                ...
                runOnUiThread(new Runnable() {
                    public void run() {
                        if (currentPosition >= mMediaDuration - 3 && mMediaDuration > 0) {
                            if (mStartAutoPlayed) {
                                return;
                            } else {
                    ...
                    public void run() {
                        if (TextUtils.isEmpty(mediaDuration)
                            ...
                            mHandler.postDelayed(new Runnable() {
                                @Override
                                public void run() {
                                    LogUtil.e(TAG, "Get media duration failed, retry later."
                                            + "Duration:" + mediaDuration + "intLength:"
                                            + mMediaDuration);
                                    getMediaDuration();
                                }
                            }, RETRY_TIME);
